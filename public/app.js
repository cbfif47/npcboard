var todomvc = angular.module('todomvc', ['firebase', 'ui.bootstrap']);
...
todomvc.controller('TodoCtrl', function TodoCtrl($scope, $firebase, $firebaseAuth) {
    ...
    function authDataCallback(authData) {
      if (authData) {
        	...
        	$scope.todos = $firebase(fireRef.orderByChild("priority")).$asArray();      
	} else {
	...
	$scope.addTodo = function(){
		...
		//find bottom of list for lowest priority
                var items = $scope.todos.filter(filterStatus('new'));
	        var bottom = items[0].priority;
